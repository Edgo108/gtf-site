...
// dans public/markers/ — voir public/markers/README.md). Choix
// entièrement automatique, basé sur le statut : un labo "potentiel"
// affiche toujours l'icône "?" (categorie inconnue ou pas encore
// confirmée), quelle que soit la catégorie renseignée ou non.
export function labMarkerIconUrl(marker: {
  statut: LabStatut;
  categorie: LabCategorie | null;
}): string {
  if (marker.statut === "potentiel" || !marker.categorie) {
    return "/markers/potentiel.png";
  }
...
export function isLabStatut(value: unknown): value is LabStatut {
  return value === "actif" || value === "raided" || value === "potentiel";
}
...
export type LabMarker = {
  id: string;
  // Nullable uniquement quand statut === "potentiel" (labo repéré mais
  // dont le type n'est pas encore confirmé) — appliqué en base par
  // lab_markers_categorie_required_unless_potentiel.
  categorie: LabCategorie | null;
  statut: LabStatut;
